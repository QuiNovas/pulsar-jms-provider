...
import com.echostreams.pulsar.jms.common.*;
...
public class PulsarSession extends AbstractSession {
        ...
        try {
            ...
            if (commitPuts) {
                ...
                synchronized (pendingPuts) {
                    if (!pendingPuts.isEmpty()) {
                        ...
                        LOGGER.debug(this + " - COMMIT [PUT] " + pendingPuts.size() + " message(s)");
                        ...
                        try {
                            for (int i = 0; i < pendingPuts.size(); i++) {
                                ...
                                PulsarDestination targetDestination = getPulsarDestination(message);
            ...
            if (queuesWithGet != null && putFailure == null) {
                ...
                if (deliveredMessageIDs != null) {
                    ...
                    LOGGER.debug(this + " - COMMIT [GET] " + deliveredMessageIDs.size() + " message(s)");
                    pendingGets = transactionSet.clear(deliveredMessageIDs);
                } else {
                    // Commit the whole transaction set
                    LOGGER.debug(this + " - COMMIT [GET] " + transactionSet.size() + " message(s)");
                    pendingGets = transactionSet.clear();
                }

                for (int i = 0; i < queuesWithGet.size(); i++) {
                    PulsarQueue pulsarQueue = queuesWithGet.get(i);
                    if (pulsarQueue.remove(this, pendingGets))
                        committables.add(pulsarQueue);
                    consumedCount++;
                }
            }
    ...
    private void rollbackUpdates(boolean rollbackPuts, boolean rollbackGets, List<String> deliveredMessageIDs) throws JMSException {
        ...
        if (rollbackPuts && transacted) {
            if (!pendingPuts.isEmpty()) {
                LOGGER.debug(this + " - ROLLBACK [PUT] " + pendingPuts.size() + " message(s)");
        ...
        if (rollbackGets && transactionSet.size() > 0) {
            ...
            if (deliveredMessageIDs != null) {
                ...
                LOGGER.debug(this + " - ROLLBACK [GET] " + deliveredMessageIDs.size() + " message(s)");
                pendingGets = transactionSet.clear(deliveredMessageIDs);
            } else {
                // Rollback the whole transaction set
                LOGGER.debug(this + " - ROLLBACK [GET] " + transactionSet.size() + " message(s)");
                pendingGets = transactionSet.clear();
            }
            List<PulsarQueue> queuesWithGet = computeUpdatedQueues(pendingGets);
    ...
    private PulsarDestination getPulsarDestination(AbstractMessage message) throws JMSException {
    ...
    private List<Committable> computeLocalTargetDestinations(List<AbstractMessage> pendingPuts, List<PulsarQueue> queuesWithGet) throws JMSException {
        ...
        if (pendingPuts != null) {
            for (int i = 0; i < pendingPuts.size(); i++) {
                ...
                PulsarDestination destination = getPulsarDestination(msg);
    ...
    public final void dispatch(AbstractMessage message) throws JMSException {
        // Security
        PulsarConnection conn = (PulsarConnection) getConnection();
        LOGGER.debug(this + " [PUT] in " + message.getJMSDestination() + " - " + message);
